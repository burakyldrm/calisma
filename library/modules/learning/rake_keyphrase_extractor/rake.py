...
# the original that takes path
def build_stop_word_regex_path(stop_word_file_path):
...
def build_stop_word_regex_list(stop_word_list):
    stop_word_regex_list = []
    for word in stop_word_list:
        word_regex = '\\b' + word + '\\b'
        stop_word_regex_list.append(word_regex)
...
class Rake(object):
    def __init__(self, stop_words_list, stop_words_path, min_char_length=1, max_words_length=5, min_keyword_frequency=1):
        self.__stop_words_path = stop_words_path
        if len(stop_words_list) == 0:
            self.__stop_words_pattern = build_stop_word_regex_path(stop_words_path)
        else:
            self.__stop_words_pattern = build_stop_word_regex_list(stop_words_list)
...
if test:
    ...
    stopwordpattern = build_stop_word_regex_path(stoppath)
