...
from modules.sentiment import tr_sentiment_classification as tr_sent
import modules.prototypes.classification as clsf
from modules.sentiment import sentiment_conf as conf
import modules.dataset.twitter.twitter_preprocessing as twitter_prep
from modules.dataset import corpus_io
import modules.prototypes.token_based_multilang as tbtrans
...
def predict_tweets_polarity(model_folder, test_tweets, test_labels):
    ...
    if len(long_instances) > 0:
        statu=1
        lpredicted_labels, lprediction_map = clsf.run_saved_model_sentiment_tr_twit(model_folder, long_instances, _ltrue_labels)
...
def trainTwitterTr():
    ...
    picklefolder = BASE_DIR+ "datamodel/tr_tweets"
...
def sentimentResultTwitterTr(paramtext):
    ...
    if ypred[0]=="positive" or ypred[0]=="pos":
...
if __name__ == "__main__":
    ...
    folder = "/home/user/git/cognitus-web_new/modules/sentiment/datacvs/"
    fname = "tr_polartweets.csv"
    ...
    picklefolder = "/home/user/git/cognitus-web_new/modules/sentiment/datamodel/tr_tweets"
    modelname = "tr_tweet2_svm5"
